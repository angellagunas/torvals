...
class SidebarItem extends Component {
  ...
  getItemLink(to, icon, title, onClick, exact) {
    return (
      <NavLink exact={exact} activeClassName="is-active" to={to}>
        <span className="icon">
  ...
  getDropdownButton(to, icon, title, toggle, dropdownItems, isLvl2, index) {
    ...
    const dropdownClass = classNames('', {
      dropdown: isLvl2
        ? this.state.menuLvl2IsCollapsed
        : this.state.menuIsCollapsed,
      'is-active': isActive,
    });
  ...
  getDropdownItems(dropdown, to, icon, title, onClick, dropdownOnClick) {
    let dropdownItems = (
      <ul className={this.state.open ? '' : 'is-hidden'}>
        {dropdown.length > 0 &&
          dropdown.filter(item => item).map((e, i) => {
            if (!this.testRoles(e.roles)) return null;
            if (e.dropdown) {
              return (
                <li key={e.title.toLowerCase().replace(/\s/g, '')}>
                  {this.getDropdownItemsLvl2(
                    e.dropdown,
                    e.to,
                    e.icon,
                    e.title,
  ...
  render() {
    ...
    if (dropdown) {
      ...
      mainLink = this.getDropdownItems(
        dropdown,
        to,
        icon,
        title,
