...
class UserDetail extends Component {
  ...
  getDateCreated() {
    if (this.state.user.dateCreated) {
      return moment
        .utc(this.state.user.dateCreated)
  ...
  render() {
    ...
    if (user.roleDetail && currentUser) {
      disabledForm =
        user.roleDetail.priority <= currentUser.currentRole.priority;
    }

    var disabledRoles = false;
    if (
      user.roleDetail &&
      currentUser.currentRole.slug === 'consultor-level-3'
    ) {
      disabledRoles = true;
      disabledForm = true;
    }
    ...
    if (user) {
      ...
      if (role && role.slug === 'manager-level-1') {
        var currentOrg = user.organizations.find(item => {
          return (
            item.organization.uuid === currentUser.currentOrganization.uuid
          );
        });
      ...
      <div className="detail-page">
        <div className="level">
          ...
          <div className="level-right">
            ...
            <div className="level-item">{!disabledForm && resetButton}</div>
          </div>
        </div>
                  ...
                  <div className="card">
                    ...
                    <div className="card-content">
                      <Multiselect
                        ...
                        assignedClickHandler={this.assignedGroupOnClick.bind(
                          this
                        )}
                        disabled={disabledForm}
                      />
