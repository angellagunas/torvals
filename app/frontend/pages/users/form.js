...
class UserForm extends Component {
  ...
  render() {
    ...
    if (schema.properties.project) {
      schema.properties.project.enum = this.props.projects.map(item => {
        return item.uuid;
      });
      schema.properties.project.enumNames = this.props.projects.map(item => {
        return item.name;
      });
    }
    if (this.props.disabled) {
      ...
      schema.properties.role.enumNames.push(
        this.state.formData.roleDetail.name
      );
    }
