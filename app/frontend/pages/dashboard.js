...
class Dashboard extends Component {
  ...
  async getProjects() {
    ...
    const outdated =
      allProjects.length > 0 ? allProjects.every(item => item.outdated) : false;
  ...
  async selectProject(e, value, project) {
    ...
    this.setState({
      allProjects:
        Object.keys(this.selectedProjects).length ===
    ...
    if (!this.state.waitingData) {
      try {
        ...
        this.setState({
          ...
          startPeriod: moment
            .utc()
            .startOf(this.rules.cycle)
            .format('YYYY-MM-DD'),
          endPeriod: moment
            .utc()
            .endOf(this.rules.cycle)
            .format('YYYY-MM-DD'),
          waitingData: false,
        });
        setTimeout(() => {
  ...
  render() {
    ...
    if (
      !user.currentOrganization.isConfigured &&
      user.currentRole.slug === 'orgadmin'
    ) {
      return <Wizard rules={this.rules} org={user.currentOrganization} />;
    }
    ...
    const graph = [
      {
        ...
        data: this.state.graphData
          ? this.state.graphData.map(item => {
              return item.prediction !== undefined ? item.prediction : null;
            })
          : [],
      },
      {
        ...
        data: this.state.graphData
          ? this.state.graphData.map(item => {
              return item.adjustment !== undefined ? item.adjustment : null;
            })
          : [],
      },
      {
        ...
        data: this.state.graphData
          ? this.state.graphData.map(item => {
              return item.sale !== undefined ? item.sale : null;
            })
          : [],
      },
      {
        ...
        data: this.state.graphData
          ? this.state.graphData.map(item => {
              return item.previousSale !== undefined ? item.previousSale : null;
            })
          : [],
      },
    ];
                                  ...
                                  <li key={item.uuid}>
                                    <a>
                                      <Checkbox
                                        ...
                                        handleCheckboxChange={(e, value) =>
                                          this.selectProject(e, value, item)
                                        }
                                        key={item.uuid}
                                        disabled={this.state.waitingData}
                                      />
              ...
              {this.state.filteredData && this.state.graphData ? (
                this.state.filteredData.length > 0 ? (
                  ...
                  <section className="section">
                    <center>
                      <h1 className="has-text-info">
                        No hay productos que mostrar, intente con otro filtro
                      </h1>
                    </center>
                  </section>
                )
