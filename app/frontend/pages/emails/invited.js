...
class EmailInviteLanding extends Component {
  ...
  async verifyToken() {
    ...
    var data;
    try {
      data = await api.post('/emails/invite/validate', tokenData);
    } catch (e) {
  ...
  async submitHandler({ formData }) {
    ...
    if (!user.organizations || user.organizations.length === 0) {
      return this.setState({
        error:
          '¡El usuario no tiene una organización asignada, No se puede iniciar sesión automáticamente!',
        loading: false,
        apiCallErrorMessage: 'message is-danger',
        formData: {
          email: '',
  ...
  render() {
        ...
        <div className={'LogIn single-form ' + this.props.className}>
          <div className="card">
            <header className="card-header">
              <p className="card-header-title">Selecciona una organización</p>
    ...
    return (
      <div className="Invited single-form">
        <div className="card">
          <header className="card-header">
            <p className="card-header-title">Hola {this.state.user.name}!</p>
            <a className="card-header-icon">
              <span className="icon">
                <i className="fa fa-angle-down" />
              </span>
