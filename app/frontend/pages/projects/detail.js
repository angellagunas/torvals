...
class ProjectDetail extends Component {
  ...
  async componentWillMount() {
    ...
    if (
      ...
      this.props.match.params.uuid !== user.currentProject.uuid
    ) {
    ...
    if (
      ...
      this.intervalConciliate = setInterval(() => {
    ...
    try {
      ...
      if (!tab && currentRole !== 'manager-level-1') {
        if (body.data.status === 'empty') {
          ...
          body.data.status === 'pendingRows' ||
          body.data.status === 'processing' ||
          body.data.status === 'conciliating'
        ) {
          tab = 'ajustes';
  ...
  async countAdjustmentRequests() {
    if (this.state.project.activeDataset) {
      var url =
        '/app/adjustmentRequests/counter/' +
        this.state.project.activeDataset.uuid;
      try {
  ...
  async handleAllAdjustmentRequest(showMessage = true) {
    ...
    await this.handleAdjustmentRequest(
      pendingDataRowsArray,
      showMessage,
      finishAdjustments
    );
    this.setState({
  ...
  async handleAdjustmentRequest(obj, showMessage, finishAdjustments = false) {
    ...
    let rows = productAux.filter(item => {
      return item.newAdjustment && item.isLimit;
    });
    try {
      var res = await api.post('/app/rows/request', {
        rows: rows,
        finishAdjustments: finishAdjustments,
        cycle: cycle.uuid,
        dataset: this.state.project.activeDataset.uuid,
      });
      if (
        currentRole === 'manager-level-1' ||
        currentRole === 'manager-level-2'
  ...
  render() {
            ...
            <div className="column">
              <article className="message is-warning">
                ...
                <div className="message-body has-text-centered is-size-5">
                  Necesitas tener asignado al menos un centro de venta para ver
                  esta sección, ponte en contacto con tu supervisor.
                </div>
              </article>
            </div>
          </div>
          ...
          <div className="columns">
            <div className="column">
              <article className="message is-warning">
                ...
                <div className="message-body has-text-centered is-size-5">
                  Necesitas tener asignado al menos un canal para ver esta
                  sección, ponte en contacto con tu supervisor.
                </div>
              </article>
            </div>
          </div>
    ...
    const tabs = [
        ...
        content: (
          <TabAdjustment
            ...
            handleAdjustmentRequest={row => {
              this.handleAdjustmentRequest(row);
            }}
            handleAllAdjustmentRequest={() => {
              this.handleAllAdjustmentRequest();
            }}
      ...
      },
      {
        ...
        title: 'Datasets',
        hide: testRoles(
          'manager-level-1, consultor-level-2, manager-level-2, consultor-level-3'
        ),
        reload: true,
        content: (
          <TabDatasets
                      ...
                      <div className="field is-grouped">
                        ...
                        <div className="control">
                          <DeleteButton
                            ...
                            message={
                              '¿Estas seguro de querer eliminar este Proyecto?'
                            }
                            hideIcon
    ...
    let options = (
      <button
        className={'button is-primary no-hidden'}
    ...
    if (
      !testRoles(
        'consultor-level-3, consultor-level-2, manager-level-1, manager-level-2'
      ) &&
      this.state.actualTab === 'aprobar'
    ) {
      consolidarButton = (
    ...
    return (
      <div>
              ...
              <div className="columns">
                <div className="column">
                  <article className="message is-warning">
                    ...
                    <div className="message-body has-text-centered is-size-5">
                      Este proyecto aún no contiene datasets, ponte en contacto
                      con tu supervisor.
                    </div>
                  </article>
                </div>
        ...
        >
          <p>
            Este proyecto se encuentra usando una version pasada de reglas de
            negocio, ¿Desea actualizarlo?
          </p>{' '}
          <br />
          <div className="field is-grouped">
            <div className="control">
            ...
            <div className="control">
              <button
                className="button"
                onClick={this.hideModalOutdated.bind(this)}
                type="button"
              >
                Cancelar
              </button>
...
export default Page({
  ...
  exact: true,
  roles:
    'consultor-level-3, analyst, orgadmin, admin, consultor-level-2, manager-level-2, manager-level-1, manager-level-3',
