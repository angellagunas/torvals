...
class TabApprove extends Component {
  ...
  async getAdjustmentRequests() {
    if (this.props.project.activeDataset) {
      let url =
        '/app/adjustmentRequests/dataset/' +
        this.props.project.activeDataset.uuid;
      try {
    ...
    return [
      {
        ...
        abbr: (() => {
          if (
            ...
            return (
              <div
                className={this.state.remainingItems > 0 ? '' : 'is-invisible'}
              >
                <Checkbox
                  label="checkAll"
                  handleCheckboxChange={e =>
                    this.checkAll(!this.state.selectedAll)
                  }
                  key="checkAll"
      ...
      },
      {
        ...
        formatter: row => {
          if (row.cycle) {
            return (
              moment.utc(row.cycle.dateStart).format('MMMM') +
              ' - ' +
              row.cycle.cycle
            );
          }
      ...
      },
      {
        ...
        formatter: row => {
          let percentage =
            ((row.newAdjustment - row.lastAdjustment) / row.lastAdjustment) *
            100;
  ...
  searchDatarows() {
    const items = this.state.dataRows
      .map(item => {
  ...
  async handleChange(obj) {
    let index = this.state.dataRows.findIndex(item => {
