...
class TabAnomalies extends Component {
  ...
  async handleChange(obj) {
    ...
    if (res.data) {
      obj.edited = true;
      let index = this.state.anomalies.findIndex(item => {
  ...
  checkAll = check => {
    ...
    for (let item of this.state.anomalies) {
      if (check) selected[item.uuid] = item;
  ...
  toggleButtons() {
    ...
    if (Object.keys(this.state.selected).length > 0) disable = false;
              ...
              currentRole !== 'consultor-level-2' && (
                <div className="column is-narrow is-margin-top-20">
                  <button
                    ...
                    disabled={
                      !!this.state.isRestoring || this.state.disableButton
                    }
                    type="button"
