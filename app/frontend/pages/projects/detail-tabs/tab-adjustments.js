...
class TabAdjustment extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      nextProps.project.uuid !== this.props.project.uuid
    )
      return;

    if (
      nextProps.selectedTab === 'ajustes' &&
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.project.status === 'adjustment' &&
      prevProps.project.status !== 'adjustment'
    ) {
      this.clearSearch();
      this.getFilters();
    }
  ...
  async getFilters() {
    if (
      this.props.project.activeDataset &&
      this.props.project.status === 'adjustment'
    ) {
      ...
      try {
        ...
        if (currentRole !== 'manager-level-1') {
          cycles = cycles.map((item, key) => {
            return (item = {
              ...
              name:
                moment.utc(item.dateStart).format('MMMM D') +
                ' - ' +
                moment.utc(item.dateEnd).format('MMMM D'),
            });
          });
        } else {
          cycles = cycles.map((item, key) => {
        ...
        for (let fil of Object.keys(res)) {
          ...
          if (res[fil][0]) formData[fil] = res[fil][0].uuid;
        }
  ...
  async getDataRows() {
    ...
    if (this.props.project.cycleStatus !== 'rangeAdjustment') adjustment = 0;
    ...
    for (let row of data) {
      ...
      if (row.adjustment !== row.lastAdjustment || row.adjustmentRequest) {
        ...
        if (this.state.generalAdjustment > 0) {
          var maxAdjustment = Math.ceil(
            row.prediction * (1 + this.state.generalAdjustment)
          );
          var minAdjustment = Math.floor(
            row.prediction * (1 - this.state.generalAdjustment)
          );

          row.isLimit =
            row.adjustmentForDisplay > maxAdjustment ||
            row.adjustmentForDisplay < minAdjustment;
        }
      }
    }
                    ...
                    <button
                      className={
                        this.state.disableButtons
                          ? 'button is-outlined disabled-btn'
                          : 'button is-outlined'
                      }
        ...
        <div className="column is-narrow show-rows">
          <Checkbox
            ...
            handleCheckboxChange={(e, value) =>
              this.showRows('showAdjusted', value)
            }
          ...
          <Checkbox
            ...
            handleCheckboxChange={(e, value) =>
              this.showRows('showNotAdjusted', value)
            }
            ...
            <p>
              <span>
                {this.state.byWeek
                  ...
                  : this.state.selectedCheckboxes.size}{' '}
              </span>
  ...
  toggleButtons() {
    ...
    if (this.state.selectedCheckboxes.size > 0) disable = false;
  ...
  async handleChange(obj) {
    ...
    for (let row of rowAux) {
      ...
      if (this.state.generalAdjustment > 0) {
        row.isLimit =
          row.newAdjustment > maxAdjustment ||
          row.newAdjustment < minAdjustment;
      }
      ...
      if (
        ...
        this.notify(
          <p>
            ...
            ¡Debes pedir una solicitud de ajuste haciendo clic sobre el ícono
            rojo o el botón finalizar!
          </p>,
          5000,
          toast.TYPE.WARNING
        );
  ...
  async searchDatarows() {
    ...
    const items = this.state.dataRows.filter(item => {
      ...
      const searchStr = `${item.productName} ${item.productId} ${
        item.channel
      } ${item.salesCenter}`;
  ...
  setAlertMsg() {
    ...
    if (currentRole === 'consultor-level-3' || ajuste === 0) {
      ...
      return (
        <span>
          Modo Ajuste {this.state.generalAdjustment * 100} % permitido
        </span>
      );
    }
  }
  ...
  async updateSalesTable(row) {
    ...
    for (let i = 0; i < salesTable.length; i++) {
      if (row.semanaBimbo === parseInt(salesTable[i].week)) {
        let price = Math.abs(
          (row.adjustmentForDisplay - row.lastLocalAdjustment) *
            row.productPrice
        );
  ...
  async downloadReport() {
    ...
    try {
      ...
      this.notify(
        '¡Se ha generado el reporte correctamente!',
        5000,
        toast.TYPE.SUCCESS
      );
    } catch (e) {
  ...
  toggleIndicators = () => {
    this.setState({
      indicators:
        this.state.indicators === 'indicators-show'
          ? 'indicators-hide'
          : 'indicators-show',
    });
  };
  ...
  makeFilters() {
    ...
    if (zeroFilters === numfilters) {
      ...
      if (
        ...
        msg =
          '¡No tienes catálogos asignados! Por favor contacta a tu supervisor.';
      }
  ...
  getCallback() {
    if (this.state.prices) {
      return function(label, index, labels) {
        return (
          '$' +
          label.toFixed(2).replace(/./g, (c, i, a) => {
  ...
  getTooltipCallback() {
    if (this.state.prices) {
      return function(tooltipItem, data) {
        ...
        let yVal =
          '$' +
          tooltipItem.yLabel.toFixed(2).replace(/./g, (c, i, a) => {
            return i && c !== '.' && (a.length - i) % 3 === 0 ? ',' + c : c;
          });
        return label + yVal;
      };
  ...
  render() {
    ...
    if (this.props.project.status === 'processing') {
      return (
        <div className="section has-text-centered subtitle has-text-primary">
          Se están obteniendo las filas para ajuste, en un momento más las podrá
          consultar.
          <Loader />
        </div>
      );
    ...
    if (this.props.project.status === 'pendingRows') {
      return (
        <div className="section has-text-centered subtitle has-text-primary">
          Se está preparando al proyecto para generar un dataset de ajuste,
          espere por favor.
          <Loader />
        </div>
      );
    ...
    const graphData = [
      {
        label: 'Predicción',
        color: '#187FE6',
        data: this.state.salesTable.map((item, key) => {
          return item.prediction.toFixed(2);
        }),
      },
      {
        label: 'Ajuste',
        color: '#30C6CC',
        data: this.state.salesTable.map((item, key) => {
          return item.adjustment.toFixed(2);
        }),
      },
      {
      ...
      <div>
        ...
        <div className="section level selects">
          <div className="columns is-multiline is-mobile">
            ...
            {this.state.filters && this.makeFilters()}
          </div>
        </div>
          ...
          <div
            className={
              this.state.indicators === 'indicators-hide'
                ...
                : ' level-item has-text-centered no-border'
            }
          >
            <div>
              <img
                src="/app/public/img/grafica.png"
                className={
                  this.state.indicators === 'indicators-hide' ? '' : 'disapear'
                }
              />
                        ...
                        {this.state.salesTable.map((item, key) => {
                          return (
                            <tr key={key}>
                              ...
                              <td className="has-text-centered">
                                {this.state.prices && '$'}{' '}
                                {((this.state.prevData[key] || {}).sale || 0)
                                  .toFixed(2)
                                  .replace(/./g, (c, i, a) => {
                                    return i &&
                                      c !== '.' &&
                ...
                <div className="panel-block">
                  ...
                  this.state.salesTable.length > 0 ? (
                    <Graph
                      ...
                      labels={this.state.salesTable.map((item, key) => {
                        return 'Periodo ' + item.period[0];
                      })}
                      tooltips={{
                        mode: 'index',
                        intersect: true,
                    ...
                    this.state.filteredData.length > 0 ? (
                      <ProductTable
                        ...
                        adjustmentRequestCount={
                          Object.keys(this.state.pendingDataRows).length
                        }
                        handleAdjustmentRequest={row => {
                          this.props.handleAdjustmentRequest(row);
                        }}
                        handleAllAdjustmentRequest={() => {
