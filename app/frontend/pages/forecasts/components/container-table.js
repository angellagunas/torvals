...
class ContainerTable extends Component {
    ...
    this.setState(
      {
        ...
        predictionsFormatted: body.data.map(item => {
          ...
          if (data.prediction !== data.adjustment) {
            percentage =
              ((data.adjustment - data.prediction) * 100) / data.prediction;
          }
          ...
          return {
            ...
            isLimit:
              Math.abs(percentage) >=
              this.props.forecast.project.adjustment * 100,
            uuid: item.uuid,
          };
        }),
      },
      function() {
        if (this.state.predictionsFormatted.length > 0) {
          ...
          this.setState(
            {
              schema,
              predictionsFiltered: this.state.predictionsFormatted.map(
                item => item
              ),
              weekSelected: schema.weeks.enum[0],
              days: this.handleDaysPerWeek(
                schema.weeks.groupedByWeeks[schema.weeks.enum[0]]
              ),
              daySelected:
                schema.weeks.groupedByWeeks[
                  Math.min(...Object.keys(schema.weeks.groupedByWeeks))
                ][0],
              weeksOptions: {
  ...
  selectDay(e) {
    this.setState(
      { daySelected: e.target.innerText.replace(/\s/g, '') },
      this.filterData
    );
  }
  ...
  selectWeek(e) {
    if (e) {
      this.setState(
        {
          days: this.handleDaysPerWeek(
            this.state.schema.weeks.groupedByWeeks[e]
          ),
          weekSelected: e,
  ...
  async handleChange(data) {
    ...
    const prediction = this.state.predictions.find(item => {
      return data.uuid === item.uuid;
    });
    var maxAdjustment = Math.ceil(
      prediction.data.prediction * (1 + project.adjustment)
    );
    ...
    data.isLimit =
      data.adjustment >= maxAdjustment || data.adjustment <= minAdjustment;
    ...
    if (
      ...
      if (data.adjustment > maxAdjustment || data.adjustment < minAdjustment) {
        this.notify(
          ' ¡No te puedes pasar de los límites establecidos!',
          5000,
          toast.TYPE.ERROR
        );
        return false;
