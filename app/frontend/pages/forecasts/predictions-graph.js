...
class PredictionsGraph extends Component {
  ...
  async loadGraphData() {
    ...
    this.setState({
      ...
      graphData: body.data.map(item => ({
        ...item,
        ds: item[item.hasOwnProperty('ds') ? 'ds' : 'sds'],
      })),
    });
  }
    ...
    if (!loading && Object.keys(graphData).length === 0) {
      barGraph = (
        <div className="notification has-text-centered">
          No hay datos Generales de esta predicción
        </div>
      );
    }
    ...
    return (
      <div className="card">
        <header className="card-header">
          <p className="card-header-title">Predictions Graph</p>
        </header>
        ...
        <div className="card-content">
          <div className="columns is-multiline is-relative">
            ...
            <div className="column is-relative">{barGraph}</div>
          </div>
        </div>
      </div>
    );
