...
class ForecastCompare extends Component {
  ...
  async getForecasts() {
    ...
    if (forecasts === undefined) {
      this.props.history.push(
        '/forecast/detail/' + this.props.match.params.uuid
      );
      return;
    }
    ...
    catalogs = await this.getCatalogs(
      Object.values(catalogs).map(item => {
        return item.uuid;
      })
    );
  ...
  handleSort(e) {
    ...
    if (e === 'externalId') {
      if (this.state.sortAscending) {
        ...
        sorted.sort((a, b) => {
          return parseInt(b.product[e]) - parseInt(a.product[e]);
        });
      }
    } else {
      if (this.state.sortAscending) {
  ...
  async getGraph() {
    ...
    let url =
      '/app/forecastGroups/graph/compare/' + this.props.match.params.uuid;
    try {
      let res = await api.post(url, {
        engines: this.state.forecasts.map(item => {
          return item.engine.uuid;
        }),
  ...
  async getTable() {
    ...
    let url =
      '/app/forecastGroups/graph/compare/table/' + this.props.match.params.uuid;
    try {
      let res = await api.post(url, {
        engines: this.state.forecasts.map(item => {
          return item.engine.uuid;
      ...
      console.log(e);
      this.notify(
        'Error obteniendo tabla ' + e.message,
        5000,
        toast.TYPE.ERROR
      );
    }
