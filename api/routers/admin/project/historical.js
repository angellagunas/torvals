...
module.exports = new Route({
  ...
  handler: async function (ctx) {
    ...
    if (data.salesCenter) {
      const agenciaName = project.activeDataset.getSalesCenterColumn().name
      ...
      requestBody[agenciaName] = salesCenter.externalId
    }

    if (data.channel) {
      const channelName = project.activeDataset.getChannelColumn().name
      const channel = await Channel.findOne({uuid: data.channel})
      ctx.assert(channel, 404, 'Canal no encontrado')
      requestBody[channelName] = channel.externalId
    }

    if (data.product) {
      const productName = project.activeDataset.getProductColumn().name
      const product = await Product.findOne({uuid: data.product})
      ctx.assert(product, 404, 'Producto no encontrado')
      requestBody[productName] = data.product
    }
    if (data.category) {
