...
class CreateBarGraph extends Component {
  ...
  drawPaths (svg, data, x, y) {
    let upperOuterArea = d3.area()
      .x(function (d) { return x(d.ds) || 1 })
      .y0(function (d) { return y(d.yhat_upper) })
      .y1(function (d) { return y(d.yhat) })
      .curve(d3.curveBasis)
  ...
  createChart (data, markers = []) {
    ...
    const y = d3
      .scaleLinear()
      .range([chartHeight, 0])
      .domain([d3.min(data, function (d) { return d.yhat_lower - 10 }), d3.max(data, function (d) { return d.yhat_upper })])
