...
class Projects extends Component {
    ...
    return [
      {
        ...
        formatter: (row) => {
          return (
            <Link to={'/projectsv2/detail/' + row.uuid}>
              {row.name}
            </Link>
          )
        }
      ...
      },
      {
        'title': 'Actions',
        formatter: (row) => {
          return <Link className='button' to={'/projectsv2/detail/' + row.uuid}>
            Detalle
          </Link>
        }
      }
  ...
  finishUp (object) {
    ...
    this.props.history.push('/admin/projectsv2/detail/' + object.uuid)
  }
...
const branchedProjects = branch({projects: 'projectsv2'}, Projects)
