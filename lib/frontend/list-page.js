...
export default function (options = {}) {
  class ListPage extends Component {
    ...
    async load () {
      let res = await api.get(options.baseUrl, {page: 1, limit: 10})
      this.context.tree.set(options.branchName, {
        page: 1,
    ...
    getImportComponent () {
      if (options.import) {
        ...
        return (
          <div className='card-header-select'>
            <button className='button is-primary' onClick={() => this.showModalImport()}>
          ...
          <div className='field'>
            <div className='control has-icons-right'>
              <input
                ...
                type='text'
                value={this.state.filters.general || this.state.filters.name}
  ...
  const BranchedListPage = branch({branch: options.branchName}, injectIntl(ListPage))
  ...
  return {
    ...
    asRouterItem: () => {
      return <Route exact={options.exact} path={env.PREFIX + options.path} render={props => {
        ...
        const Component = withTracker(BranchedListPage, options.tracker)
    ...
    asRouterItemList: (key) => {
      return <Route key={key} exact={options.exact} path={env.PREFIX + options.path} render={props => {
        ...
        if (ctx.hasError) {
          console.log('should error with', ctx.error)
          trackPage(location, { hasError: hasError, error: ctx.error })
          return <div>Has error {ctx.error.status}</div>
        }
