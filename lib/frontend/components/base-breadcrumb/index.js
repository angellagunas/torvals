...
class Breadcrumb extends Component {
  ...
  handleClick (event, pathObj) {
    event.preventDefault()
    this.props.history.push(pathObj.path)
    if (pathObj.onClick) {
      pathObj.onClick(pathObj.path)
    }
  }
  ...
  getLinkPath (pathObj) {
    if (pathObj.path && pathObj.path !== '') {
      return (
        <a href={pathObj.path} aria-current={pathObj.current ? 'page' : ''} onClick={e => this.handleClick(e, pathObj)}>
          {pathObj.label}
        </a>
      )
    }
