...
const TableHeader = (props) => {
  ...
  return (
    <th className={rowClass} onClick={callbackEvent}>{body || props.children} {icon}</th>
  )
}
  ...
  getColumns () {
    if (this.props.columns) {
      ...
      this.props.columns.map((col, i) => {
        if (col.formatter) {
          return columns.push(<td key={i} className={col.centered ? 'has-text-centered' : ''}>{col.formatter(this.props.row) || col.default}</td>)
        }
    ...
    if (this.props.data) {
      return (
        this.props.data.map((row, i) => (
          <BodyRow className={row.edited ? 'is-edited' : ''} key={i} row={row} columns={this.props.columns} />
        ))
      )
    }
    return null
