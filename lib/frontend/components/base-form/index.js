...
const CheckboxWidget = (props) => {
  ...
  return (
    <div className={'checkboxwidget ' + props.options.className}>
      ...
      <div className='select'>
        <select
          ...
          onBlur={
            onBlur &&
            (event => {
              const newValue = this.getValue(event, multiple)
              onBlur(id, this.processValue(schema, newValue))
            })
          }
          onFocus={
            onFocus &&
            (event => {
              const newValue = this.getValue(event, multiple)
              onFocus(id, this.processValue(schema, newValue))
            })
          }
          onChange={event => {
            const newValue = this.getValue(event, multiple)
            onChange(this.processValue(schema, newValue))
          }}>
          {!multiple && schema.type !== 'boolean' && !schema.default && <option value=''>{placeholder}</option>}
...
function CustomHorizontalFieldTemplate (props) {
  const {
    id,
    classNames,
    label,
  ...
  if (id === 'root') {
    ...
    return (
      <div className={classNames}>
        <label
          className='field-label label is-normal'
          htmlFor={id}
        >
          {label}{required ? '*' : null}
        </label>
...
class BaseForm extends Component {
  transformErrors (errors) {
    return errors.map(error => {
      if (error.name === 'required') {
        error.message = 'Es un campo requerido'
  ...
  render () {
    let customField = CustomFieldTemplate
