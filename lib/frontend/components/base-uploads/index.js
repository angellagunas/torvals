...
class UploadDataset extends Component {
  ...
  componentDidMount () {
    let ResumableField = new Resumablejs({
      target: this.props.url || env.API_HOST + '/api/admin/upload/',
      maxFiles: '1',
    ...
    ResumableField.on('fileError', (file, errorCount) => {
      this.errorHandler({
        message: (
          ...
          '!. Upload cancelled. You should try again.'
        )
      })
    ...
    return (
      <div className='card'>
        <Prompt
          when={isUploading}
          message={location => (
            `There is an upload in progress, are you shure you want to leave this page?`
          )}
        />
