...
class UploadDataset extends Component {
  ...
  componentWillMount () {
    var dropzoneId = 'upload_file_dropzone'

    window.addEventListener('dragenter', function (e) {
      if (e.target.className.indexOf(dropzoneId) === -1) {
        e.preventDefault()
  ...
  startUpload () {
    if (/\s/.test(this.state.fileList[0].fileName)) {
      this.setState({
        error: 'El nombre del archivo no puede contener espacios en blanco',
        apiCallErrorMessage: 'has-text-danger'
      })
      return
    }
      ...
      <div className='card'>
        ...
        <div className='card-content'>
          <div className='columns is-centered'>
            <div className='column is-8 is-narrow has-text-centered upload_file_dropzone'
              id='upload_file_dropzone'>
              <div
                className={fileList.length === 1
                  ? 'file is-boxed is-centered is-large is-hidden is-info upload_file_dropzone'
                  : 'file is-boxed is-centered is-large is-info upload_file_dropzone'}
              >
                <label className='file-label upload_file_dropzone'>
                  <input
                    hidden
                    ...
                    name='upload_file'
                  />
                  <span className='file-cta upload_file_dropzone'>
                    <span className='file-icon upload_file_dropzone'>
                      <i className='fa fa-cloud-upload upload_file_dropzone' />
                    </span>
                    <span className='file-label upload_file_dropzone'>
