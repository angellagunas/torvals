...
class BodyRow extends Component {
  ...
  componentDidMount () {
    ...
    if (this.props.row.hasOwnProperty('selected')) {
      isRowSelected = this.props.row.selected
    }
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.row.hasOwnProperty('selected')) {
      if (nextProps.row.selected !== this.state.isRowSelected) {
        this.setState({ isRowSelected: nextProps.row.selected })
      }
    }
  }
  ...
  async onEnter (row, col, value, event) {
    var original
    if (event.charCode === 13) {
      if (row.hasOwnProperty(col.property)) {
        ...
        original = row[col.property]
        row[col.property] = value
        row.edited = true
      }
      this.hideInput()
      const res = await this.props.handleChange(row, value)

      if (!res) {
        row[col.property] = original
        row.edited = false
      }
    }
  }
  ...
  getColumns () {
    if (this.props.columns) {
      ...
      this.props.columns.map((col, i) => {
        ...
        if (col.formatter) {
          rowComponent = col.formatter(this.props.row, this.state) || col.default
        }
        ...
        if (col.editable) {
          ...
          columns.push(<td key={i}>{rowComponent || col.default}</td>)
        }
      })
  ...
  getrow () {
    ...
    const rowClass = classNames(this.props.className, {
      ...
      'was-edited': !this.props.row.edited && !this.props.row.selected && this.props.row.wasEdited
    })

    if (this.props.selectable && this.props.noDouble) {
      return <tr
        className={rowClass}
    ...
    else if (this.props.selectable && !this.props.noDouble) {
      return <tr
        className={rowClass}
        onClick={(event) => this.handleSingleClickItem(this.props.row)}
      ...
      return (
        this.props.data.map((row, i) => (
          <BodyRow
            ...
            setRowsToEdit={this.props.setRowsToEdit}
