...
class UpdateProfileForm extends Component {
  ...
  async selectAllLanguages () {
    ...
    for (let language of languages.data) {
      result[language.uuid] = this.formatTitle('language.' + language.name)
    }
    this.setState({
      languages: result
    })
  ...
  render () {
    ...
    const schema = {
      ...
      required: ['name', 'email'],
      properties: {
        name: {type: 'string', title: this.formatTitle('profile.name')},
        email: {type: 'string', title: this.formatTitle('profile.email')},
        job: {type: 'string', title: this.formatTitle('profile.job')},
        phone: {type: 'string', title: this.formatTitle('profile.phone')},
        language: {
          type: 'string',
          title: this.formatTitle('language.language'),
          enum: Object.keys(this.state.languages),
          enumNames: Object.values(this.state.languages)
        },
        profile: {type: 'string', title: this.formatTitle('profile.img'), format: 'data-url'}
      }
    }
      ...
      <div className='is-fullwidth'>
        {this.state.accountOwner.uuid === this.state.user.uuid &&
          <div className='accountOwner'>
            <strong>{this.formatTitle('profile.owner')}</strong>
          </div>
        }
        {this.state.user.isVerified &&
          <div className='accountOwner'>
            <strong>{this.formatTitle('profile.verified')}</strong>
          </div>
        }
        <BaseForm schema={schema}
          ...
          <div className={this.state.apiCallMessage}>
            <div className='message-body is-size-7 has-text-centered'>
              {this.formatTitle('profile.saved')}
            </div>
          </div>
