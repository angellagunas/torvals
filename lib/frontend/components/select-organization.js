...
class SelectOrganizationForm extends Component {
  ...
  render () {
    ...
    return (
      <nav className={this.props.className ? 'navbar ' + this.props.className : 'navbar bg-whitesmoke'} role='navigation' aria-label='dropdown navigation'>
        <div className='navbar-item has-dropdown is-hoverable'>
          {this.getDefaultObject()}
          {user.organizations.length > 1 ? <div className='navbar-dropdown'>
            {listData}
          </div> : null}
        </div>
      </nav>
