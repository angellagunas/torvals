...
async function request (options) {
  ...
  try {
    ...
    var reqDataLog = JSON.parse(JSON.stringify(reqData))

    if (reqDataLog.response && reqDataLog.response._items && reqDataLog.response._items.length > 50) {
      reqDataLog.response._items = _.sampleSize(reqDataLog.response._items, 50)
      reqDataLog.response._items.push('and ' + (reqData.response._items.length - 50) + ' more ')
    }
