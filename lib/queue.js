...
const Queue = class {
  ...
  run (argv) {
    ...
    if (this.isFile) {
      let numWorkers = 1
      if (config.queueWorkers.multiple) {
        numWorkers = config.queueWorkers.numWorkers
      }

      console.log(`[${taskName}] ${numWorkers} workers`)
      this._queue.process(numWorkers, this._fn)
    } else {
